...
const RoleSelectorDropdown = ({
	...
	const handleOnClick = (accountRoleItems) => {
		...
		if (isPartnerMember) {
			...
			const updatedMemberRoles = memberRoles.filter(role => role.active);
		...
		if (!isPartnerMember) {
			const accountRoleItem = Object.values(accountRoleItems).filter(role => role.active);
			...
			trigger={
				...
				>
					<div className="text-truncate">
						{i18n.translate(getKebabCase(selectedAccountRoleName[0]))
							? i18n.translate(
						...
						<>
							<RadioRoles
								...
								onClick={() => {
									...
									Object.keys(radioOptions).forEach(roleLabel => { newObject[roleLabel].active = roleLabel === key })
									
									newObject.partnerMemberRoles.roles = newObject.partnerMemberRoles.roles.map((role) => (
										{...role, active: false}
									))
							...
							{accountRole.roles.map((role, accountRoleIndex) => (
								<ClayCheckbox
									checked={role.active}
									className="pr-6"
									disabled={role.disabled || !radioOptions.partnerMemberRoles.active}
									key={accountRoleIndex}
									onClick={() => {
										const newObject = {...radioOptions}
							
										const partnerMemberRole = newObject.partnerMemberRoles.roles[accountRoleIndex];

   										partnerMemberRole.active = !partnerMemberRole.active;

										if (partnerMemberRole.active) {
											Object.keys(newObject).forEach(key => {
												if (key !== "partnerMemberRoles") {
													newObject[key].active = false;
												}
											});
										}
										...
										const activeMemberRoles = (role) => role.active;
