...
export class PortletConfigurationPermissionsPage {
    ...
    constructor(page : Page) {
        ...
        this.permissionsFrame = page.frameLocator('iframe[title="Permissions"]');
        this.permissionsMenuItem = page.getByRole('menuitem', { name: 'Permissions' });
        this.productMenuPage = new ProductMenuPage(page);

        this.clearLink = this.permissionsFrame.getByLabel('Clear');
        this.ownerRoleCell = this.permissionsFrame.getByRole('cell', {name : 'Owner'});
        this.resultsBanner = this.permissionsFrame.getByText('Results Found for');
        this.saveButton = this.permissionsFrame.getByRole('button', { name: 'Save' });
        this.searchBar = this.permissionsFrame.getByPlaceholder('Search for');
        this.siteMemberRoleCell = this.permissionsFrame.getByRole('cell', {name : 'Site Member'});
        this.successMessage = this.permissionsFrame.getByText('Success:Your request completed successfully.');
    }
