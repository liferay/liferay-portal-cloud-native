...
export const test = mergeTests(
	...
	loginTest(), systemSettingsPageTest
);
...
test('LPD-30561 Cookie Banner Cookie Policy Page', async ({
	...
	await test.step('Enable Explicit Cookie Consent Mode', async () => {
		const explicitCookieConsentModeButton = page.getByLabel('Explicit Cookie Consent Mode');
	...
	await test.step('Go to Cookie Policy page', async () => {
		...
		const objectDefinitionPortlets = await page.locator(
		...
		for (const objectDefinitionPortletIndex in objectDefinitionPortlets) {
			const objectDefinitionPortlet = objectDefinitionPortlets[objectDefinitionPortletIndex];

			await expect(objectDefinitionPortlet.locator('.dnd-thead')).toBeVisible({
				timeout: 100 * 1000,
			});

			const tableRows = await objectDefinitionPortlet.locator('.dnd-tr').all();
