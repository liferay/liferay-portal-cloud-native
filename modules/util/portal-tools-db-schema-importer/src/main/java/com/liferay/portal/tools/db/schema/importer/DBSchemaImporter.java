...
public class DBSchemaImporter {
	...
	private static class DBSchemaImporterProcess {

		public DBSchemaImporterProcess(
			String path, DataSource sourceDataSource,
				...
				try (ResultSet resultSet = preparedStatement1.executeQuery()) {
					while (resultSet.next()) {
						for (int i = 0; i < sourceColumnsName.size(); i++) {
							...
							_getAndSetColumn(
								columnName, i + 1, preparedStatement2,
								resultSet,
		...
		private void _getAndSetColumn(
			...
			else if ((sourceType == Types.BOOLEAN) ||
					 (sourceType == Types.BIT)) {
		...
		private void _preprocessSQL(String sqlTemplate) throws Exception {
			...
			try (UnsyncBufferedReader unsyncBufferedReader =
					new UnsyncBufferedReader(
						new UnsyncStringReader(sqlTemplate))) {
		...
		private void _safeCopyTable(
			...
			try (Connection sourceConnection =
					_sourceDataSource.getConnection();
				Connection targetConnection =
					_targetDataSource.getConnection()) {
		...
		private void _setColumn(
			...
			else if ((targetType == Types.BIT) ||
					 (targetType == Types.BOOLEAN)) {

				preparedStatement.setBoolean(
					index, GetterUtil.getBoolean(value));
			}
		...
		private final Map<String, List<String>> _sourceTableColumns =
		...
		private final Map<String, List<String>> _targetTableColumns =
			new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
